...
def cluster_kmeans(fps, n_clusters=5):
    '''
    运用kmeans算法进行聚类
    :param fps:
    :param n_cluster:
    ...
    kmeans = KMeans(n_clusters=n_clusters, random_state=20).fit(df)
    df['file_path'] = file_path
    df['label'] = kmeans.labels_
    # cluster_df = df[['file_path', 'label']]
    return df
